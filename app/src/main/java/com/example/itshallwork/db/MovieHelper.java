...
import static com.example.itshallwork.db.DatabaseContract.MovieColumns.DATE;
import static com.example.itshallwork.db.DatabaseContract.MovieColumns.OVERVIEW;
import static com.example.itshallwork.db.DatabaseContract.MovieColumns.POSTER;
import static com.example.itshallwork.db.DatabaseContract.MovieColumns.TITLE;
import static com.example.itshallwork.db.DatabaseContract.MovieColumns.VOTE;
import static com.example.itshallwork.db.DatabaseContract.TABLE_MOVIE;
...
public class MovieHelper {
    ...
    public ArrayList<MovieItem> getAllMovies() {
        ...
        if (cursor.getCount() > 0) {
            do {
                ...
                note.setVoteCount(cursor.getDouble(cursor.getColumnIndexOrThrow(VOTE)));
                arrayList.add(note);
                cursor.moveToNext();
    ...
    public MovieItem getMovieById(int id) {
        Cursor cursor = database.query(
                DATABASE_TABLE,
                new String[]{_ID, TITLE, OVERVIEW, DATE, POSTER, VOTE},
                _ID + "=?",
        ...
        if (cursor.getCount() > 0) {
            ...
            movieItem.setOverview(cursor.getString(cursor.getColumnIndex(OVERVIEW)));
            ...
            movieItem.setVoteCount(cursor.getDouble(cursor.getColumnIndex(VOTE)));
    ...
    public long insertMovie(MovieItem movie) {
        ...
        args.put(VOTE, movie.getVoteCount());
